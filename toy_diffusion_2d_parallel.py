...
def generate_combinations(base_dict, keys, values):
    ...
    list_of_dicts = [dict(zip(keys, combination)) for combination in all_combinations]
    # Add the base dictionary to each dictionary in the list
    update_list=[]
    for i in range(len(list_of_dicts)):
        base_dict.update(list_of_dicts[i])
        update_list.append(base_dict.copy())
    return update_list
